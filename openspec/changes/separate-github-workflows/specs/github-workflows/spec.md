...
Orquesta dos workflows de GitHub Actions claramente separados para validar cada cambio y publicar releases de forma segura, sin acoplar CI de tests con el ciclo de publicación a npm.

## ADDED Requirements

### Requirement: CI workflow ejecuta tests en push, PR y merge a main

El workflow de CI SHALL ejecutar la suite de Jest en cada evento que pueda integrar código a `main`, garantizando que ningún cambio llegue a `main` sin pasar tests en la matrix soportada.

#### Scenario: Push a main dispara CI
- **WHEN** se hace `git push` a `main` (incluye merge de PR vía GitHub)
- **THEN** el workflow `ci.yml` se inicia en `ubuntu-latest`, `macos-latest` y `windows-latest` con `node` 18, 20 y 22, ejecuta `npm ci` y luego `npm test` con `--experimental-vm-modules`
...
El workflow de publish SHALL publicar el paquete `osfetch` a `https://registry.npmjs.org` únicamente cuando se publica una Release en GitHub, usando el secreto `NPM_TOKEN`, gateado por tests y con provenance.
...
- **THEN** el workflow `publish.yml` se ejecuta en `ubuntu-latest` con `node 22`, hace `checkout`, `setup-node` con `registry-url: https://registry.npmjs.org`, `npm ci`, `npm test`, y si pasa, ejecuta `npm publish --provenance --access public` con `NODE_AUTH_TOKEN: ${{ secrets.NPM_TOKEN }}`

#### Scenario: Publish falla si tests fallan
- **WHEN** el paso `npm test` falla en `publish.yml`
...
Los dos workflows SHALL estar en ficheros distintos con triggers, permisos y caching no acoplados, de modo que cambios en CI no afecten publish y viceversa.
...
- **THEN** usa `actions/checkout@v4` y `actions/setup-node@v4` con `cache: 'npm'` (o `pnpm` si el proyecto migra) sin duplicar lógica entre ficheros
